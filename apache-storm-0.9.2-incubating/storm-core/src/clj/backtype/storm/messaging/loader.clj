...
;;Added a function here for testing purposes

(defn read_msg_drop_probability[filename]    
    (let [value_read 
        (if  (.exists (clojure.java.io/file filename))
          (do 
                     
            (def temp (slurp filename))
            (read-string temp)
                  ...
                  (let [packet (.next iter)
                      ...
                      (if (= task -1)
                         (do (log-message "Receiving-thread:[" storm-id ", " port "] received shutdown notice")
                           ...
                           (reset! closed  true))
                         (when packet 
                           (.add batched [task message])
                           ;;(do
                           ;;  (let [msg_drop_probability (read_msg_drop_probability (str Constants/TIMEOUT_FILE_BASE_DIR "Config.txt"))
                           ;;        ]
                           ;;    (if (= msg_drop_probability 0)
                           ;;      (do
                           ;;        (.add batched [task message])
                           ;;        (println "Not dropped")
                           ;;        )
                                ;;else
                           ;;     (do
                           ;;      (let [rand-number-max (int (/ (float 1) (float msg_drop_probability)))
                           ;;           gen-rand-number (rand-int rand-number-max)
                           ;;           ] 
                           ;;        (if (not (= gen-rand-number 1))
                           ;;          (do
                           ;;          (.add batched [task message])
                           ;;          (println "not dropped")
                           ;;          )
