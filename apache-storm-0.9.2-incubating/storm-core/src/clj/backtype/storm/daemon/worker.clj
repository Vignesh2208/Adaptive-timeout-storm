...
(ns backtype.storm.daemon.worker
  ...
  (:use [backtype.storm.tuple])
  (:require [backtype.storm.daemon [executor :as executor]])
  (:import [java.util.concurrent Executors])
  (:import [java.util ArrayList HashMap List LinkedHashMap LinkedList])
...
(defn mk-transfer-local-fn [worker]
  (let [short-executor-receive-queue-map (:short-executor-receive-queue-map worker)
        ...
        tuple-rate-map (:incoming-tuple-rate-map worker)
        tuple-count-map (:incoming-tuple-count-map worker)
        start-time-map (:dst_task-start-time-map worker)
        task-inter-arrival-time-map (:task-inter-arrival-time-map worker)
        task-queuing-delay-start-map(:task-queuing-delay-start-map worker)
        
        
        contxt (worker-context worker)]
    (fn [tuple-batch]
          ...
          (let [q (short-executor-receive-queue-map short-executor)]
            (if q
              ...
              (do 
                ...
                (let [pairs_temp (.get pairs 0)
                      dst_task (.get pairs_temp 0)
                      tupld (.get pairs_temp 1)
                      ]
                  
                       ;;; Task inter arrival stuff
                       (if (.containsKey task-inter-arrival-time-map (str dst_task))
                         (do
                           (let [^LinkedList base-list (.get task-inter-arrival-time-map (str dst_task))
                                 prev-time (.get base-list 1)
                                 curr-time (System/nanoTime)
                                 time-diff (- curr-time prev-time)
                                 ]
                                (.set base-list 0 time-diff)
                                (.set base-list 1 curr-time)
                         ...
                         ;;else
                         (do 
                          (let [base-list (java.util.LinkedList.)
                                curr-time (System/nanoTime)
                                ]
                               (.add base-list 0)
                               (.add base-list curr-time)
                               (.put task-inter-arrival-time-map (str dst_task) base-list)
                          )
                       ...
                       ;;; Task queuing delay start stuff
                       
                        (if (.containsKey task-queuing-delay-start-map (str dst_task))
                        (do
                         (let [^LinkedList base-list (.get task-queuing-delay-start-map (str dst_task))
                              curr-time (System/nanoTime)
                             ]
                             (if (< (.size base-list) 1024)
                             (.add base-list curr-time)
                             )
                        ...
                        (do 
                         (let [base-list (java.util.LinkedList.)
                              ...
                              (.add base-list curr-time)
                              (.put task-queuing-delay-start-map (str dst_task) base-list)
                         )
                       ...
                       ;;;
                           ...
                           (let [deserializer (KryoTupleDeserializer. (:storm-conf worker) (worker-context worker))
                                 tupl-temp (.get pairs_temp 1)
                                 ;;tupl (if (instance? Tuple tupl-temp)
                                 ;;        tupl-temp
                                 ;;        (.deserialize deserializer tupl-temp)
                                
                                 ;;     )
                                 tupl 0
                                 ]
                             (if (not (instance? Tuple tupl)) ;; Simply change this to activate the if statement
                               (do
                                 
                                 (if (.containsKey tuple-count-map (str dst_task))
                                   (do
                                     
                                     (let [tempcount (.get tuple-count-map (str dst_task))
                                           tempcount2 (+ tempcount 1) 
                                           ]
                                           (.put tuple-count-map (str dst_task) tempcount2)
                                   ...
                                   (do
                                     ...
                                     (.put tuple-count-map (str dst_task) 1)
                                     (.put start-time-map (str dst_task) (.get tuple-rate-map "start-time"))
                                     
                                   )  
                        
                                 )
                                 (let [;;start-time (.get tupl-rate-map "start-time")
                                       start-time (.get start-time-map (str dst_task))
                                       curr-time (current-time-millis)
                                       time-diff (time-delta-ms start-time)
                                       ;;streamId (.getSourceStreamId tupl)     
                                       ]
                                       ;;(if (= streamId "__tick")
                                       (if (> time-diff 30000)
                                         (do
                                           (.put tuple-count-map (str dst_task) 0)
                                           (.put tuple-rate-map (str dst_task ) (float 0))
                                           (.put start-time-map (str dst_task) curr-time)
                                         )
                                         ;;else part 
                                         (if (not= time-diff 0)
                                           (if (.containsKey tuple-rate-map (str dst_task))
                                             (do
                                               ...
                                               (let [tempcount (.get tuple-count-map (str dst_task))
                                                     ...
                                                     (.put tuple-rate-map (str dst_task) (float (* (/ (float tempcount) (float time-diff)) 1000)))
                                             ...
                                             (do
                                               ...
                                               (.put tuple-rate-map (str dst_task) (* (/ (float 1) (float time-diff)) 1000))
                                             )  
                                 ...
                                 ;;(println "Tuple received. Curr count Map = " tuple-count-map "Curr rate Map = " tuple-rate-map)
                                 ;; Generate a new tuple here
                                 
                                 ;;;;(let [orig-values (java.util.ArrayList. (.getValues tupl))
                                       ;;orig-values (.getValues tupl)
                                 ;;;;      taskId (.getSourceTask tupl)
                                 ;;;;      streamId (.getSourceStreamId tupl)
                                 ;;;;      msgId (.getMessageId tupl)   
                                 ;;;;      new-msg-id (MessageId/makeId (.. tupl getMessageId getAnchorsToIds))
                                 ;;;;      values_size (.size orig-values)
                                 ;;;;      new-values (.set orig-values (- values_size 3) (.get tuple-rate-map (str dst_task)))
                                 ;;;;      ;;new-values (.set orig-values (- values_size 3) 0)
                                 ;;;;      serializer (KryoTupleSerializer. (:storm-conf worker) contxt)
                                 ;;;;      new-tuple (TupleImpl. contxt
                                 ;;;;               ;;values
                                 ;;;;               orig-values
                                 ;;;;               taskId
                                 ;;;;               streamId
                                 ;;;;               new-msg-id)
                                 ;;;;      new-pairs (java.util.ArrayList.)
                                 ;;;;      ;;^List new-list (java.util.ArrayList.)
                                  ;;;;     new-list (java.util.ArrayList.)
                                       
                                       
                                  ;;;;     ]
                                       
                                  ;;;;     ;;(println "For Dst task : " dst_task " From Source Task : " taskId " Before Orig tupl : " tupl " On streamId : " streamId )
                                  ;;;;     ;;(.setValues tupl orig-values)
                                  ;;;;     ;;(.setProcessSampleStartTime new-tuple (.getProcessSampleStartTime tupl))
                                  ;;;;     ;;(.setExecuteSampleStartTime new-tuple (.getExecuteSampleStartTime tupl))
                                  ;;;;     ;;(println "Ack value Before : " (.getAckVal new-tuple) " Ack value of orig tuple : " (.getAckVal tupl)) 
                                       
                                  ;;;;     (.updateAckVal new-tuple (.getAckVal tupl))
                                  ;;;;     ;;(println "Ack value After : " (.getAckVal new-tuple))
                                  ;;;;     ;;(println "Old values : " (.getValues tupl) " New values : " orig-values)
                                  ;;;;     (.add new-list dst_task)
                                  ;;;;     (.add new-list new-tuple)
                                  ;;;;     (.add new-pairs new-list)
                                  ;;;;     ;;(println "For Dst task : " dst_task " From Source Task : " taskId " After Orig tupl : " tupl " On streamId : " streamId )
                                  ;;;;     ;;(println "For Dst task : " dst_task " New tuple created : " new-tuple )
                                  ;;;;     ;;(println "Old pairs : " pairs)
                                  ;;;;     ;;(println "New pairs created : " new-pairs)
                                  ;;;;     (if (or (= streamId "__ack_ack") (= streamId "__ack_fail") (= streamId "__ack_init"))
                                  ;;;;       (disruptor/publish q pairs)
                                 
                              )
                                  ;;;;     ;;(disruptor/publish q pairs)
                                       
                                  ;;;;     ;;(println "New values : " orig-values)
                                  ;;;;     ;;(println "New values created : " new-values)
                                                       
                               ;;else
                               ;;nothing
                               
                               (do
                                 ;;(println "Did not receive a tuple")
...
;; new additions
(def input-tuple-rate-map
  (let [temp (java.util.HashMap.)]
        (.put temp "start-time" (current-time-millis))
   temp) 
  
)
(def input-tuple-count-map
  (let [temp (java.util.HashMap.)]
  temp) 
  
)

(def dst_task-start-time-map
  (let [temp (java.util.HashMap.)]
  temp) 
...
(defn worker-data [conf mq-context storm-id assignment-id port worker-id]
  (let [cluster-state (cluster/mk-distributed-cluster-state conf)
    ...
    (recursive-map
      ...
      :timeout-timer (mk-halting-timer "timeout-timer")
      ...
      ;;new additions
      :incoming-tuple-rate-map input-tuple-rate-map
      :incoming-tuple-count-map input-tuple-count-map
      :dst_task-start-time-map dst_task-start-time-map
      :task-queuing-delay-start-map (java.util.HashMap.)
      :task-inter-arrival-time-map (java.util.HashMap.)
...
(defserverfn mk-worker [conf shared-mq-context storm-id assignment-id port worker-id]
  ...
  (let [worker (worker-data conf shared-mq-context storm-id assignment-id port worker-id)
        ...
        shutdown* (fn []
                    ...
                    (cancel-timer (:timeout-timer worker))
        ...
        ret (reify
             ...
             (waiting? [this]
               (and
                 ...
                 (timer-waiting? (:timeout-timer worker))
                 ))
             )]
