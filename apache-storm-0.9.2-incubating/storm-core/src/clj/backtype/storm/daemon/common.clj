...
(ns backtype.storm.daemon.common
  ...
  (:import [backtype.storm.generated StormTopology
            InvalidTopologyException GlobalStreamId StreamInfo])
...
(def ACKER-TIMEOUT-DISPATCH-STREAM-ID acker/ACKER-TIMEOUT-DISPATCH-STREAM-ID)
...
(def SYSTEM-TIMEOUT-STEAM-ID Constants/SYSTEM_TIMEOUT_STREAM_ID)
  ...
  (let [num-executors (if (nil? (storm-conf TOPOLOGY-ACKER-EXECUTORS)) (storm-conf TOPOLOGY-WORKERS) (storm-conf TOPOLOGY-ACKER-EXECUTORS))
        acker-bolt (thrift/mk-bolt-spec* (acker-inputs ret)
                                         ...
                                         {ACKER-ACK-STREAM-ID (thrift/direct-output-fields ["id"])
                                          ...
                                          ACKER-TIMEOUT-DISPATCH-STREAM-ID (thrift/direct-output-fields ["timeout-value"])
                                          }
                                         :p num-executors
    ...
    (dofor [[_ bolt] (.get_bolts ret)
           ...
           (do
             (.put_to_streams common ACKER-ACK-STREAM-ID (thrift/output-fields ["id" "ack-val" "Source-lambda" "Source-mu" "Start-timestamp" "Stop-timestamp" "inter-arrival" "queuing-delay"]))
             (.put_to_streams common ACKER-FAIL-STREAM-ID (thrift/output-fields ["id" "Source-lambda" "Source-mu" "Start-timestamp" "Stop-timestamp" "inter-arrival" "queuing-delay"]))
             ))
    (dofor [[_ spout] (.get_spouts ret)
      ...
      (do
        ...
        (.put_to_streams common Constants/SPOUT_COMPUTED_TIMEOUT_STREAM_ID (thrift/output-fields ["spout-computed-timeout-value"]))
        ...
        (.put_to_inputs common
                        ...
                        (thrift/mk-direct-grouping))
        (.put_to_inputs common
                        (GlobalStreamId. ACKER-COMPONENT-ID ACKER-TIMEOUT-DISPATCH-STREAM-ID)
                        (thrift/mk-direct-grouping))
        ))
...
(defn add-lambda-mu-fields-to-all-configured-streams! [^StormTopology topology]
    ;;(println "Function add-lambda-mu-fields-to-all-configured-streams! called") 
  ;;(locking
    (doseq [[_ component] (all-components topology)
            :let [common (.get_common component)]]
      ...
      (do
        ;;(println "New component configuration")
        
        (def stream-map (.get_streams common))
      
        (doseq [keyval stream-map] 
        
          (let [output_fields_list (java.util.ArrayList. (.get_output_fields (val keyval)))
                white_list (java.util.ArrayList.)
                
                ]
            
            ;;(locking output_fields_list
              (do
                (.add white_list ACKER-INIT-STREAM-ID)
                (.add white_list ACKER-ACK-STREAM-ID)
                (.add white_list ACKER-FAIL-STREAM-ID)
                (.add white_list ACKER-TIMEOUT-DISPATCH-STREAM-ID)
                (.add white_list ACKER-INIT-STREAM-ID)
                (.add white_list SYSTEM-STREAM-ID)
                (.add white_list SYSTEM-TICK-STREAM-ID)
                (.add white_list METRICS-STREAM-ID)
                (.add white_list SYSTEM-TIMEOUT-STEAM-ID)
                
                (if (not (.contains white_list (key keyval)))
                ;;(if (not (.contains output_fields_list "end-of-values"))
                 (do
                   ;;(println "Before: Output_fields_list = " output_fields_list) 
                   (let [output_fields_list_new
                     (doto 
                      output_fields_list
                      (.add "start-timestamp")
                      (.add "stop-timestamp")
                      ;;;;(println "Added lambda mu")
                      ;;(.add "end-of-values")
                      ;;;;(println "Added end-of-values")
                     )]
               
             
                     
                     ;;;;(println "After : Output fields list = " output_fields_list_new)
              
                     (.put_to_streams common (key keyval) (StreamInfo. output_fields_list_new true))
                     ;;;;(println "Before adding Fields to stream" (key keyval) "New values are " updated_stream_info)
                     ;;;;(.put_to_streams common (key keyval) updated_stream_info)
                    )
                 )
                ;;)
                )
          ...
          ;;(println "Fields added to stream" (key keyval))
        ...
        ;;(doseq [keyval (.get_streams common)]
        ;;       (println "Updated configuration")
        ;;      (prn (key keyval) (val keyval) ))
  ...
  ;;)
  ;;(println "Function add-lambda-mu-fields-to-all-configured-streams! Ended")
)
...
(defn add-system-components! [conf ^StormTopology topology]
  (let [system-bolt-spec (thrift/mk-bolt-spec*
                          ...
                          {SYSTEM-TICK-STREAM-ID (thrift/output-fields ["rate_secs"])
                           ...
                           SYSTEM-TIMEOUT-STEAM-ID (thrift/output-fields ["description"])}                          
                          :p 0
...
(defn system-topology! [storm-conf ^StormTopology topology]
  ...
  (let [ret (.deepCopy topology)]
    ...
    ;;(add-lambda-mu-fields-to-all-configured-streams! ret)
    ret
    ))
