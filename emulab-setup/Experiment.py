...
def print_usage() :
	...
	print "python Experiment.py -t <topology_name> -r <initial_rate> -s <step_size> -n <number of iterations> -m <timeout-mode> -p <msg-drop-sample-size> -f <sample-size-multiplicative-factor>"
...
def check_arg_types(topology_name, timeout_mode, init_rate, step_size, N, sample_size, mf) :
	...
	if timeout_mode != "NORMAL" and timeout_mode != "MM1" and timeout_mode != "HT" and timeout_mode != "END_TO_END" :
...
def submit_topology(STORM_BIN_PATH,TOPOLOGY_PATH,TOPOLOGY_ID,topology_name,curr_rate,timeout_mode,sample_size) :

	print "Submitting new topology : ", topology_name
	print "Curr rate = ", curr_rate
	print "sample_size = ", sample_size
	
	subprocess.call([STORM_BIN_PATH,"jar",TOPOLOGY_PATH,TOPOLOGY_ID,topology_name,str(curr_rate),str(timeout_mode),str(sample_size)])
...
while i < N :
	...
	if topology_name == "all" :
		...
		submit_topology(STORM_BIN_PATH,TOPOLOGY_PATH,WORD_COUNT_TOPOLOGY_ID,"WordCount",curr_rate,correct_timeout_mode,sample_size)
		print "Iteration number : ", i + 1

		submit_topology(STORM_BIN_PATH,TOPOLOGY_PATH,ROLL_COUNT_TOPOLOGY_ID,"RollingTop",curr_rate,correct_timeout_mode,sample_size)
		print "Iteration number : ", i + 1

		submit_topology(STORM_BIN_PATH,TOPOLOGY_PATH,SINGLE_JOIN_TOPOLOGY_ID,"SingleJoin",curr_rate,correct_timeout_mode,sample_size)
		print "Iteration number : ", i + 1
